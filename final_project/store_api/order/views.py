...
from .stock_client import StockServiceError, confirm_reservation, reserve_stock
...
class PlaceOrderView(LoginRequiredMixin, View):
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        empty, no items removed since checkout was rendered), then
        atomically creates the order and asks stock_api to reserve the
        books for it. If stock_api reports insufficient stock (or is
        unreachable), the whole transaction is rolled back and the user is
        ...
        try:
            with transaction.atomic():
                ...
                order.reservation_id = reserve_stock(
                ...
                order.save(update_fields=["reservation_id"])

                cart.clear()
        except StockServiceError as exc:
            messages.error(request, str(exc))
            logger.warning(
                "Order placement failed: stock service rejected the order",
                error=str(exc),
                user=request.user.username,
            )
            return redirect("order:cart")
        ...
        if order.payment_method == PaymentMethod.CARD.value:
            # Reservation is confirmed later, once Stripe confirms payment
            # (see stripe_webhook) - not here, since payment hasn't happened yet.
            return redirect("order:stripe", order_id=order.pk)

        # Cash orders have no separate payment step, so the reservation is
        # confirmed (stock permanently deducted) right away.
        try:
            confirm_reservation(order.reservation_id)
        except StockServiceError:
            logger.exception(
                "Failed to confirm stock reservation for cash order",
                order_id=order.pk,
...
async def create_checkout_session(request: HttpRequest, order_id: int) -> HttpResponse:
    ...
    try:
        session = await sync_to_async(stripe.checkout.Session.create)(
            ...
            success_url=(
                request.build_absolute_uri(reverse("order:order_success"))
                + "?checkout_session_id={CHECKOUT_SESSION_ID}"
            ),
            cancel_url=(
                request.build_absolute_uri(reverse("order:checkout"))
                + "?error=epayment_error"
            ),
        )
        if not session.url:
...
async def stripe_webhook(request: HttpRequest) -> HttpResponse:
    ...
    # Stripe's event/session objects only support item access (event["id"])
    # and `in`, not dict's .get() - they're typed SDK objects, not plain dicts.
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if order.reservation_id:
            try:
                await sync_to_async(confirm_reservation)(order.reservation_id)
            except StockServiceError:
                logger.exception(
                    "Failed to confirm stock reservation after payment",
                    order_id=order.pk,
                    reservation_id=order.reservation_id,
                )
