...
from order.stock_client import StockServiceError, cancel_reservation
...
def cancel_stale_orders():
    """Cancel CARD orders stuck unpaid for too long (customer abandoned
    checkout) and release their stock reservation in stock_api."""
    cutoff = timezone.now() - timedelta(hours=settings.ORDER_STALE_HOURS)
    ...
    for order in stale_orders:
        try:
            cancel_reservation(order.reservation_id)
        except StockServiceError:
            continue  # stock_api is unreachable, retry on the next run
